...
var SistemaGestaoClientes = () => {
  ...
  const saveClient = async () => {
    ...
    let result;
    if (editingClient) {
      result = await supabase.from("gerenciador_clientes").update(clientData).eq("id", editingClient.id);
    } else {
      result = await supabase.from("gerenciador_clientes").insert([clientData]);
    }
    if (result.error) {
